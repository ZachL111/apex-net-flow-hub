...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-apex-net-flow-detail.ps1

node dist/tests/domainReview.test.js

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-apex-net-flow-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-apex-net-flow-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-apex_net_flow_hub-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
